...
function nativeCacheRoot(options2 = {}) {
  ...
  if (platform === "linux") {
    ...
    return (0, import_node_path11.resolve)(base, "releaseway", "npm-actions", "native", "sha256");
  }
  if (platform === "darwin") {
...
function cachedExecutableName(target) {
  const name = import_node_path11.posix.basename(target.executable);
  if (!name || name === "." || name === "..") {
    throw new Error(`Invalid cached executable name: ${target.executable}`);
  }
  return name;
}
async function validateCacheEntry(directory, target) {
  try {
    const metadata = JSON.parse(
      await (0, import_promises2.readFile)((0, import_node_path11.join)(directory, "metadata.json"), "utf8")
    );
    if (metadata.schema !== 1 || metadata.assetSha256 !== target.sha256 || metadata.asset !== target.asset || metadata.sourceExecutable !== target.executable || typeof metadata.executableSha256 !== "string" || !/^[0-9a-f]{64}$/.test(metadata.executableSha256) || typeof metadata.executableFile !== "string" || metadata.executableFile !== cachedExecutableName(target)) {
      return void 0;
    }
    const executable = (0, import_node_path11.join)(directory, metadata.executableFile);
...
async function acquireCacheLock(root, finalDirectory, target, options2) {
  const lockDirectory = (0, import_node_path11.join)(root, `.${target.sha256}.lock`);
...
async function promote(temp, finalDirectory, target) {
  try {
...
async function prepareNativeExecutable(manifest, target, options2 = {}) {
  ...
  const finalDirectory = (0, import_node_path11.join)(root, target.sha256);
  ...
  try {
    ...
    const temp = await (0, import_promises2.mkdtemp)(
      (0, import_node_path11.join)(root, `.${target.sha256}.tmp-`)
    );
    try {
      ...
      const archivePath = (0, import_node_path11.join)(temp, "asset");
      await (0, import_promises2.writeFile)(archivePath, archiveBytes, { mode: 384 });
...
async function findNativeManifest(launcherPath) {
  let current = (0, import_node_path12.dirname)((0, import_node_path12.resolve)(launcherPath));
  while (true) {
    const manifestPath = (0, import_node_path12.resolve)(current, ".releaseway", "native.json");
    try {
...
// src/native/launcher/runtime.ts
async function resolveLauncherExecutable(launcherPath, target, prepare = prepareNativeExecutable) {
...
async function runNativeLauncher(options2 = {}) {
  const launcherPath = options2.launcherPath ?? import_node_process3.default.argv[1];
  if (!launcherPath) {
...
// src/native/launcher/main.ts
void runNativeLauncher().then((status) => {
