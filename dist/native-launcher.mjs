...
import { join, posix as posix2, resolve } from "node:path";
...
function nativeCacheRoot(options2 = {}) {
  const env = options2.env ?? process2.env;
  const platform = options2.platform ?? process2.platform;
  const home = options2.home ?? homedir();
  if (platform === "linux") {
    const base = env.XDG_CACHE_HOME && env.XDG_CACHE_HOME.length > 0 ? env.XDG_CACHE_HOME : join(home, ".cache");
...
function cachedExecutableName(target) {
  const name = posix2.basename(target.executable);
  if (!name || name === "." || name === "..") {
...
async function prepareNativeExecutable(manifest, target, options2 = {}) {
  ...
  try {
    ...
    try {
      ...
      await writeFile(archivePath, archiveBytes, { mode: 384 });
      ...
      await writeFile(executablePath, executableBytes, { mode: 493 });
      if ((options2.platform ?? process2.platform) !== "win32") {
        await chmod(executablePath, 493);
      }
      const metadata = {
        schema: 1,
...
async function findNativeManifest(launcherPath) {
  let current = dirname2(resolve2(launcherPath));
  while (true) {
    const manifestPath = resolve2(current, ".releaseway", "native.json");
    try {
...
function defaultReport() {
  try {
    return process3.report?.getReport();
  } catch {
    return void 0;
  }
}
...
function detectNativeTarget(options2 = {}) {
  const platform = options2.platform ?? process3.platform;
  const arch = options2.arch ?? process3.arch;
...
async function runNativeLauncher(options2 = {}) {
  const launcherPath = options2.launcherPath ?? process4.argv[1];
  if (!launcherPath) {
  ...
  const runner = options2.spawn ?? spawnSync2;
  const result = runner(
    resolved.executable,
    [...options2.args ?? process4.argv.slice(2)],
    {
      stdio: "inherit",
      env: process4.env
    }
